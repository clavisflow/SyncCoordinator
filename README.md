...
| URL | 機能 |
|---|---|
| `/` | 全同期ルールの配線表示、同期対象数、処理状況、最近のコンフリクト |
| `/systems` | システムの登録・編集・同期対象設定、個別／全体の一時停止、暗号化したDB接続情報の保存、接続テスト |
| `/routes` | 同期ルール一覧、同期対象設定、新規作成、編集 |
| `/routes/{id}` | 送信元／送信先、片方向／双方向、競合の判定単位、既定の競合ポリシー |
| `/routes/{id}/database-setup` | 業務DBへ作成する同期用テーブル／TriggerのSQL確認、ダウンロード、反映確認、有効化 |
| `/routes/{id}/mappings` | 実DBの列型・長さを取得し、列、キー、競合ポリシー、方向別の値変換、固定値、変換プレビューを設定 |
| `/conflicts` | 競合履歴一覧 |
| `/conflicts/{id}` | base、incoming、current、adopted、適用ポリシーの比較 |
| `/operations` | Queue Checkpoint、Inbox、システムイベント、設定変更履歴 |
| `/notifications` | Webhook通知先、通知イベント、署名シークレットの設定 |
| `/settings` | 表示言語、管理者パスワード変更、Workerのポーリング間隔／バッチ件数、管理DBの保持日数、クリーンアップ |
| `/help` | `docs/user-guide.md`を正本としてHTML表示する操作マニュアル兼ヘルプ |
| `/account/password` | ログイン中の管理者パスワード変更。管理設定へ戻る導線を持つ |

システム・同期ルール・マッピングの更新は`ICoordinatorAdminService`、管理設定の更新は`IManagementSettingsService`経由で行い、Blazorコンポーネントには同期判断を実装しません。同一システム間ルールと重複項目ポリシーはCoreで拒否します。設定保存時は`ConfigurationAudit`へ変更履歴を追加します。

SyncCoordinatorの設定は管理DBに保存され、Workerが各ループで読み直すため再起動なしで反映されます。システム管理の全体一時停止は、すべての同期ルールについて新しい同期処理とCheckpoint更新を止めます。各システムの個別一時停止状態は保持されるため、全体停止を解除しても個別停止中のシステムは再開しません。全体一時停止中もWebhook配信と管理DBの自動クリーンアップは継続します。

管理DBの自動クリーンアップは複数Worker間でリースを取得し、1日1回だけ実行します。保持日数を`0`にすると、その種類は自動・手動とも削除しません。削除対象は完了済みInbox、通知成功履歴、再試行を終了した通知失敗履歴、確認済みシステムイベント、設定変更履歴です。処理中・保留・失敗中のInbox、未確認のシステムイベント、コンフリクト履歴、Checkpoint、Snapshot、同期設定は削除しません。手動実行では同じ条件の削除候補件数を確認してから実行できます。
...
保留競合は詳細画面から採用値を選び、Workerへ安全な解決要求を登録できます。競合以外の失敗・保留Inboxは現時点では参照専用です。
...
RDB Connectorは、管理DBで同期対象になっているシステムのProviderと暗号化接続情報から動的に生成します。Workerは処理周期ごとにscopeを作り直すため、管理画面で追加・変更したシステムと接続情報は再起動せず次の周期から反映されます。Worker側にシステム別のConnector一覧や業務DB接続文字列を重複設定する必要はありません。
...
EF Core migrationは`SyncCoordinator.Infrastructure/Persistence/Migrations`に含まれます。対象環境では最新migrationまで適用してください。
...
Workerへは管理DB接続文字列`ConnectionStrings__coordinator-db`を環境変数またはWindows Service用の保護された構成ソースから渡します。業務DB接続情報は管理画面から暗号化して管理DBへ保存します。WebとWorkerが別サービスアカウントの場合は共有Key Ringを設定してください。サービスアカウントには各業務DBで必要最小限のqueue read、対象行read/write、`SyncAppliedMessage` read/write権限を付与します。
...
- `docs/decisions/0015-coordinator-schema-normalization.md`
- `docs/decisions/0016-community-manual-conflict-resolution.md`

## TODO

現時点の意図的な未実装は、失敗・保留データの手動再実行、比較・修復、監査エクスポート、SMTPメール通知です。Pro候補はTeams等のサービス固有連携、競合の一括・自動解決、承認フロー、高度な通知テンプレートとフィルターです。Community版のRDB Connector、汎用Webhook、個別の手動競合解決、方向別値変換、書込み前検証、運用イベント記録は実装済みです。
...
ログイン後は「管理設定」の「表示・アカウント」から通常変更できます。パスワード変更画面からは管理設定へ戻れます。再設定・変更時はCookieの世代番号を更新し、以前のログインCookieを無効にします。同期設定、履歴、暗号化したDB接続情報は変更しません。初期設定、再設定、通常変更は`ConfigurationAudit`へ記録しますが、パスワードとハッシュは監査履歴へ書きません。
...
ログイン前は認証画面のヘッダー、ログイン後は「管理設定」の「表示・アカウント」から日本語または英語へ変更できます。選択はASP.NET CoreのカルチャーCookieへ1年間保存され、画面を再読み込みして反映されます。管理画面と認証・アカウント画面は日英リソースを提供します。
