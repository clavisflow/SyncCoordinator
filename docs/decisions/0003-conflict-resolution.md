...
同期ルール既定または列マッピングの列別設定で `HoldAndNotify`、`ApplyIncomingAndNotify`、`KeepCurrentAndNotify`、`MergeAndNotify` を選べる。列別設定がない場合はルール既定を使用する。Field scope は非競合項目を保存でき、Record scope の hold はレコード全体を保存しない。競合履歴には base/incoming/current/adopted と実施ポリシーを JSON で保持する。
