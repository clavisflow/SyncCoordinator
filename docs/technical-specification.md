...
- 管理画面からの、コンフリクト以外の失敗・保留データの手動再実行
- 業務DB間の全件比較と自動修復
- 監査履歴のエクスポート
...
| `SyncCoordinator.E2ETests` | Aspire TestingとPlaywrightによるデモtopologyのE2Eテスト |
...
E2ETests --------> AppHost / Infrastructure
```

Webは同期データの配送を実行しない。接続テスト、メタデータ読取り、DB配備・検証では業務DB Connectorを使用する。設定更新はCoreの管理サービス境界を通じて行い、同期判断は`SynchronizationCoordinator`へ集約する。
...
`src/SyncCoordinator.AppHost/appsettings.Development.json`の`RunMode`で構成を切り替える。`Demo`は固定ポートと永続volumeを使うデモ一式、`E2E`は動的ポートと一時DBを使う自動テスト専用構成、`Core`はCoordinator Web、Worker、管理DBだけを起動する。`RunMode`未指定時は、後方互換の`Demo:Enabled`が`false`なら`Core`、それ以外は`Demo`として扱う。
...
関連テーブルの対象条件から外れた場合は`Delete`として扱うが、物理行削除と区別するフラグを持つ。対象外Deleteは、そのルールと同期先に以前のSnapshotが存在するEntityだけへ適用する。初めから対象外だったEntityによって、同じキーを持つ同期先固有行を削除してはならない。

最新状態の読取りで多重行などの非一時的な値契約違反を検出した場合、Connectorは検証失敗情報をメッセージへ付加する。Coordinatorはルール別Inboxを`Held`にしてCheckpointを進め、同じ送信元の後続Entityを停止させない。
...
- 削除同期と両端の削除方式
- 送信元の関連テーブル、基準テーブルとの結合列、用途、絞り込み条件および変更検知

送信元には基準テーブルに加えて、直接結合する関連テーブルを設定できる。関連テーブルの用途は、列をPayloadへ投影する`Projection`と、同期対象を絞り込む`Eligibility`である。関連テーブルの識別名はテーブル名を既定値とし、重複時は`_2`以降の連番を付ける。識別名は変更可能で、変更時は既存の同期項目参照も追随する。結合式と任意の条件式はDB述語として指定でき、`{related}`は関連テーブルの内部SQL別名、`{source}`は同期元テーブルの内部SQL別名へ展開される。結合式では両方のプレースホルダーを必須とし、複数文、SQLコメント、更新系SQLは保存時に拒否する。関連テーブルから投影する項目は送信元管理の片方向項目とし、逆方向の書込みおよび項目別コンフリクト判定の対象にしない。キー列は必ず基準テーブルに置く。

基準テーブルの変更は従来の基準トリガーで検知する。変更検知を有効にした関連テーブルでは、変更された結合値から影響する基準テーブルのEntityIdを展開し、同じQueueへ`Upsert`を登録する。Workerは処理時点のJOIN／EXISTS結果を再取得するため、Eligibility条件から外れた場合も対象EntityIdを特定できる。関連テーブルの変更検知とキー展開は現在SQL Serverだけをサポートする。

`Projection`の直接結合は、1つの基準レコードに対して最大1レコードへ解決できることを前提とする。1対多の明細は、明細側を基準テーブルとする別ルールで扱う。JOINの多段化、集約、任意SQLは現行のマッピング対象外とする。詳細は[関連エンティティ同期](related-entity-sync.md)を参照する。
...
| 関連テーブル、結合、対象条件、変更検知 | `Draft`へ戻す | 破棄 | 必要 |
...
送信元または送信先システムが個別停止中、またはマッピング保守中なら、共有Checkpointを進めずbatch全体を次周期へ延期する。無効なルールは通常のルール解決対象から外れ、そのルールのためにCheckpointを待機させない。
...
stateDiagram-v2
    ...
    Held --> WaitingForPrevious: 同一レコードの競合が重なる
    WaitingForPrevious --> Held: 先行競合の解決後も判断が必要
    WaitingForPrevious --> Completed: 再評価で競合が解消
    Held --> Completed: 手動解決を適用
    Held --> Superseded: 後続競合を優先
    WaitingForPrevious --> Superseded: 後続競合を優先
    Completed --> [*]
    Superseded --> [*]
    Held --> [*]
```
...
- `Completed`、`Held`、`Superseded`、`WaitingForPrevious`は通常のInbox取得では再取得しない。Held以降の競合解決は専用の解決処理が状態を更新する。
...
競合解決の状態はInbox状態とは別に管理する。

| 状態 | 意味 |
|---|---|
| `AwaitingDecision` | 運用者の選択待ち |
| `WaitingForPrevious` | 同じレコードの先行競合の解決待ち |
| `Pending` | 解決要求を登録済み、Workerの取得待ち |
| `Processing` | Workerが解決要求を適用中 |
| `Failed` | 解決適用に失敗し、内容の確認・再登録が可能 |
| `Resolved` | 自動判定または手動解決が完了 |
| `Superseded` | 後続競合を優先したため対応不要 |
...
| 対象 | 既定保持日数 | 削除条件 |
|---|---:|---|
| 完了済みInbox | 90 | `Completed`または`Superseded`、かつ更新日時が期限前 |
...
未配送Webhook、`Processing`／`Held`／`Failed`／`WaitingForPrevious`のInbox、未確認イベント、競合、Checkpoint、Snapshot、同期設定は削除しない。Deliveryを失ったWebhookEventはクリーンアップ時に削除する。
...
8. `20260716020948_SupersedeOverlappingConflicts`
9. `20260716031055_HybridConflictChainResolution`
10. `20260716044552_ExpandInboxStateLength`

`AddManagementSettings`で一度追加した`ResolvedConflictRetentionDays`は、現在の保持対象にコンフリクトを含めない方針へ合わせ、`RemoveResolvedConflictRetention`で削除済みである。最新のModel Snapshotにはこの列を含めない。
