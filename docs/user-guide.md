...
この文書は、SyncCoordinatorの管理画面を使って同期を設定・運用する管理者向けの操作マニュアルです。将来、管理画面にヘルプページを設ける場合も、この文書を正本として使用します。
...
同期処理履歴のエラー、システムイベント、コンフリクト履歴を確認してください。現在、要対応データを管理画面から修正・再実行する機能はありません。データまたは設定の修正方法を検討し、必要な場合は管理DB・業務DBの担当者と対応してください。

## コンフリクトを確認する

左メニューの「コンフリクト」を開きます。検知日時、ルール、対象IDで絞り込み・並べ替えができます。方向は表示のみです。表示対象は直近200件です。
...
コンフリクト画面は参照専用です。管理画面から値を選び直したり、コンフリクトを解決済みにしたり、同期を再実行したりすることはできません。
